...
//	private final String ERR1 = "Failed to submit feedback/bug report.";
	...
	private DateTimeFormatter dateTimeFormatFront;
	...
	// for REST api
	public ModelFeedback getFeedback(String refNumber)
	{
		String query = "SELECT * FROM "+TABLE_NAME+" WHERE "+COLUMN_REF_NUMBER+" = ?";
		
		ResultSetExtractor<ModelFeedback> extractor = new ResultSetExtractor<ModelFeedback>() {
			
			@Override
			public ModelFeedback extractData(ResultSet rs) throws SQLException, DataAccessException
			{
				if (rs.next() && rs.getString(COLUMN_REF_NUMBER).equals(refNumber))
				{
					...
					model.setFeedbackTitile(rs.getString(COLUMN_TITLE));
					model.setFeedbackDetail(rs.getString(COLUMN_DETAIL));
					model.setFeedbackErrorMessage(rs.getString(COLUMN_ERRORMESSAGE));
					LocalDateTime dateTime = LocalDateTime.parse(rs.getString(COLUMN_DATE), dateTimeFormat);
					model.setReportDate(dateTime.format(dateTimeFormatFront));
